...
class AppController {
    ...
    protected function setAllowedMethods($list) {
        $this->allowedMethodList = $list;
    }

    protected function getAllowedMethods($list) {
        return $this->allowedMethodList;
    }

    public function hasRole($method) {
        if(!in_array($method, $this->allowedMethodList) && ($_SESSION['user'] == null || $_SESSION['user']['role'] != 'admin')) {
            return false;
        } else {
            return true;
        }
